...
#region Filtering
        private static IQueryable<Person> GetPersonQuery(this DataBaseContext context, PersonsFilterFields filterFields)
        {
            ...
            if (filterFields.MinDaysAfterLastCall != 0) {
                ...
                persons = context.Persons.Include(p => p.Calls).Where(p => p.Calls.Count == 0 || p.Calls.Max(c => c.CallDate) <= lastCallDate).OrderBy(p=>p.FirstName);
            }
            else
            {
                persons = context.Persons.OrderBy(p => p.FirstName);
            }
            ...
            if (filterFields.NameFilter != string.Empty)
            {
                persons = persons.Where(p => filterFields.NameFilter.Equals(p.FirstName, StringComparison.OrdinalIgnoreCase) || filterFields.NameFilter.Equals(p.LastName, StringComparison.OrdinalIgnoreCase) || filterFields.NameFilter.Equals(p.Patronymic, StringComparison.OrdinalIgnoreCase));
            }
